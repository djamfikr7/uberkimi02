...
class RideService {
     ...
     * @returns {Promise<Object>} - Updated ride
     */
    static async updatePaymentStatus(rideId, paymentStatus) {
        try {
            const ride = await Ride.findByPk(rideId);
            ...
            if (!validPaymentStatuses.includes(paymentStatus)) {
                throw createError(400, `Invalid payment status: ${paymentStatus}`);
            }

            // Update payment status
            await ride.update({ paymentStatus });
     ...
     * @returns {Promise<Object>} - Updated ride
     */
    static async submitRating(rideId, userType, rating, feedback) {
        try {
            const ride = await Ride.findByPk(rideId);
            ...
            const updateData = {};
            if (userType === 'rider') {
                updateData.rideRatingRider = rating;
                if (feedback) updateData.riderFeedback = feedback;
            } else {
                updateData.rideRatingDriver = rating;
                if (feedback) updateData.driverFeedback = feedback;
            }

            // Update ride with rating and feedback
            await ride.update(updateData);

            // If both ratings are submitted, update driver's average rating
            if (ride.rideRatingRider !== null && ride.rideRatingDriver !== null && ride.driverId) {
                await this._updateDriverAverageRating(ride.driverId);
            }

            return ride;
        } catch (error) {
            console.error('Submit rating service error:', error);
            throw error;
        }
    }
     ...
     * @private
     */
    static async _updateDriverAverageRating(driverId) {
        try {
            ...
            if (ratedRides.length > 0) {
                ...
                const totalRating = ratedRides.reduce((sum, ride) => sum + ride.rideRatingDriver, 0);
                const averageRating = totalRating / ratedRides.length;

                // Update driver's average rating
                const driver = await Driver.findOne({ where: { userId: driverId } });
                if (driver) {
                    await driver.update({ driverRating: parseFloat(averageRating.toFixed(2)) });
                }
            }
        } catch (error) {
            console.error('Update driver average rating error:', error);

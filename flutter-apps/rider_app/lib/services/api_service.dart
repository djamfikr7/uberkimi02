...
class ApiService {
  ...
  Future<void> connectSocket(String token) async {
    try {
      ...
      // Handle new ride requests (for consistency, though riders don't typically receive these)
    ...
    try {
      if (_useMockData) {
        ...
        return {
          ...
          'data': {
            'ride': {
              'id': 'mock-ride-id-${DateTime.now().millisecondsSinceEpoch}',
              'status': 'requested',
      ...
      final response = await post('/rides', rideData);
      return response;
    } catch (e) {
